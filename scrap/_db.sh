...
echo "Db Build args: dest=$dest_folder_path, db-url=$db_url, db-path=$db_path, db-reset-ok=$db_reset_ok"
...
sqlite_db_has_data() {
    ...
    # Get table names
    local tables=$(sqlite3 "$db_path" "SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%';" 2>/dev/null)
    ...
    while IFS= read -r table_name; do
        if [[ -n "$table_name" ]]; then
            local count=$(sqlite3 "$db_path" "SELECT COUNT(*) FROM \"$table_name\";" 2>/dev/null)
            if [[ "$count" -gt 0 ]]; then
                return 0
            fi
        fi
...
create_schema_from_scripts() {
    ...
    local schema=$2
    local db_url=$3
    local temp_script_path="/tmp/temp-schema-$$-$(date +%s).sql"
    ...
    cleanup() {
        ...
        if [[ "$exit_code" -ne 0 ]]; then
            echo "Failed to create/initialize schema '$schema'"
        fi
        return $exit_code
    }
