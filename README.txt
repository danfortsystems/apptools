...
Environment Variables
	...
	- OBJECT_STORAGE_ROOT_URL
		Root url for S3-compatible object storage used by a project. When set to a local machine url (e.g., http://localhost:9000), enables nstallation and configuration of the MinIO container used for local object storage for testing.
...
Container Management:
		Container names are generic (minio, mailhog) for sharing across projects. Isolation is achieved through environment variables. Containers are installed based on presence of OBJECT_STORAGE_ROOT_URL (MinIO) and SMTP_HOST (MailHog).
